...
export default function Sidebar({
  ...
  const SidebarContent = ({ showCloseBtn = false }: { showCloseBtn?: boolean }) => (
    <div className={`${collapsed ? 'w-20' : 'w-64'} min-h-0 h-full bg-coal/70 backdrop-blur-xl text-white flex flex-col border-r border-white/10 transition-[width] duration-200`}>
      <div className={`flex items-center ${collapsed ? 'justify-center' : 'justify-between'} p-4 border-b border-white/10`}>
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 rounded-lg bg-gradient-to-tr from-amber-400 to-amber-600 shadow-aurora" />
          {!collapsed && <span className="font-semibold">BTech Buddy</span>}
        </div>
        {showCloseBtn ? (
          <button
                ...
                onClick={() => {
                  ...
                  className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg transition-colors ${
                    activeSection === item.label
                      ? 'bg-gradient-to-r from-amber-700/20 to-amber-400/10 border-l-4 border-amber-400 text-white'
      ...
      <div className="p-4 border-t border-white/10">
        <button
          ...
          className="w-full flex items-center gap-3 px-3 py-2 bg-gradient-to-r from-amber-500 to-amber-400 text-black rounded-lg shadow-inner-gold"
        >
